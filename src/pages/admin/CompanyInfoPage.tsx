// src/pages/admin/CompanyInfoPage.tsx
...
export function CompanyInfoPage() {
  ...
  const form = useForm<CompanyInfoForm>({
    initialValues: {
      name: '', logo_url: '', tax_code: '', description: '',
      address: '', phone: '', email: '', facebook_url: '',
      instagram_url: '', x_url: '', tiktok_url: '', youtube_url: '',
    },
  });
  ...
  // Effect để tạo preview ảnh không thay đổi
  useEffect(() => {
    if (logoFile) {
  ...
  const handleSubmit = async (values: CompanyInfoForm) => {
    setLoading(true);
    try {
      // Logic upload và upsert phức tạp giờ đây chỉ là một dòng gọi service
      const updatedInfo = await SettingsApi.saveCompanyInfo(values, logoFile);
      if (updatedInfo) {
        // Cập nhật lại form với dữ liệu mới nhất từ server (đặc biệt là logo_url)
        form.setFieldValue('logo_url', updatedInfo.logo_url);
        setLogoFile(null); // Xóa file đã chọn sau khi upload thành công
      }
